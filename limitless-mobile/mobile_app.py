...
def now_sydney():
    try:
        ...
        return datetime.now(timezone(timedelta(hours=11))).replace(tzinfo=None)

st.set_page_config(page_title="Limitless Site", page_icon="⚒️", layout="centered", initial_sidebar_state="collapsed")
...
def supa_post(table, data):
    ...
    try:
        ...
        r = requests.post(url, headers=h, data=json.dumps(data), timeout=8)
...
def supa_patch(table, match_col, match_val, data):
    ...
    try:
        ...
        h = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}",
             "Content-Type": "application/json", "Prefer": "return=minimal"}
        r = requests.patch(url, headers=h, data=json.dumps(data), timeout=8)
...
def init_db():
    with get_conn() as conn:
        ...
        conn.execute("CREATE TABLE IF NOT EXISTS clock_events (id INTEGER PRIMARY KEY AUTOINCREMENT, employee TEXT, job_id TEXT DEFAULT '', event_type TEXT, event_time TEXT, event_date TEXT, note TEXT DEFAULT '', status TEXT DEFAULT 'Pending', approved_by TEXT DEFAULT '', approved_at TEXT DEFAULT '', company_id INTEGER DEFAULT 1, synced INTEGER DEFAULT 0)")
        ...
        for ddl in [
            ...
            "ALTER TABLE clock_events ADD COLUMN synced INTEGER DEFAULT 0",
...
def sync_from_supabase(company_id):
    ...
    try:
        ...
        if emps:
            ...
            for e in emps:
                local_execute("INSERT OR REPLACE INTO employees (id,name,role,hourly_rate,active,pin,company_id) VALUES (?,?,?,?,?,?,?)",
                    (e.get("id"), e.get("name",""), e.get("role",""), e.get("hourly_rate",0), e.get("active",1), e.get("pin",""), company_id))
        jobs = supa_get("jobs", {"company_id": company_id})
        if jobs:
            local_execute("DELETE FROM jobs WHERE company_id=?", (company_id,))
            for j in jobs:
                local_execute("INSERT OR REPLACE INTO jobs (job_id,client,address,stage,company_id) VALUES (?,?,?,?,?)",
                    (j.get("job_id"), j.get("client",""), j.get("address",""), j.get("stage",""), company_id))
        assigns = supa_get("day_assignments", {"company_id": company_id})
        today = date.today().isoformat()
        for a in assigns:
            if a.get("date","") >= today:
                local_execute("INSERT OR REPLACE INTO day_assignments (id,job_id,client,employee,date,note,start_time,end_time,company_id) VALUES (?,?,?,?,?,?,?,?,?)",
                    (a.get("id"), a.get("job_id",""), a.get("client",""), a.get("employee",""), a.get("date",""), a.get("note",""), a.get("start_time",""), a.get("end_time",""), company_id))
        for ce in supa_get("clock_events", {"company_id": company_id}):
            cid = ce.get("id"); status = ce.get("status","Pending")
            if cid and status in ("Approved","Rejected"):
                local_execute("UPDATE clock_events SET status=? WHERE id=?", (status, cid))
        for ll in supa_get("labour_logs", {"company_id": company_id}):
            emp = ll.get("employee",""); wdate = ll.get("work_date",""); jid = ll.get("job_id",""); hrs = ll.get("hours",0)
            if not emp or not wdate: continue
            if not local_fetch("SELECT id FROM labour_logs WHERE employee=? AND work_date=? AND job_id=? AND hours=? AND company_id=?", (emp, wdate, jid, hrs, company_id)):
                local_execute("INSERT INTO labour_logs (work_date,job_id,employee,hours,hourly_rate,note,company_id,synced) VALUES (?,?,?,?,?,?,?,1)",
...
def sync_to_supabase(employee, company_id):
    ...
    errors = []
    try:
        for e in local_fetch("SELECT * FROM clock_events WHERE synced=0 AND employee=? AND company_id=?", (employee, company_id)):
            ok, msg = supa_post("clock_events", {"employee": e["employee"], "job_id": e["job_id"] or "", "event_type": e["event_type"], "event_time": e["event_time"], "event_date": e["event_date"], "note": e["note"] or "", "status": "Pending", "company_id": company_id})
            if ok: local_execute("UPDATE clock_events SET synced=1 WHERE id=?", (e["id"],))
            else: errors.append(f"clock: {msg}")
        for v in local_fetch("SELECT * FROM mobile_variations WHERE synced=0 AND employee=? AND company_id=?", (employee, company_id)):
            ok, msg = supa_post("mobile_variations", {"employee": v["employee"], "job_id": v["job_id"], "description": v["description"], "submitted_at": v["submitted_at"], "status": v["status"], "company_id": company_id})
            if ok: local_execute("UPDATE mobile_variations SET synced=1 WHERE id=?", (v["id"],))
            else: errors.append(f"variation: {msg}")
        for ll in local_fetch("SELECT * FROM labour_logs WHERE synced=0 AND employee=? AND company_id=?", (employee, company_id)):
            ok, msg = supa_post("labour_logs", {"work_date": ll["work_date"], "job_id": ll["job_id"] or "", "employee": ll["employee"], "hours": ll["hours"], "hourly_rate": ll["hourly_rate"], "note": ll["note"] or "", "company_id": company_id})
            if ok: local_execute("UPDATE labour_logs SET synced=1 WHERE id=?", (ll["id"],))
            else: errors.append(f"labour: {msg}")
        for ph in local_fetch("SELECT * FROM job_photos WHERE synced=0 AND company_id=?", (company_id,)):
            if not ph["photo_data"]: continue
            try:
                from PIL import Image; import io
                img = Image.open(io.BytesIO(ph["photo_data"])); img.thumbnail((1200,1200))
                buf = io.BytesIO(); img.save(buf, format="JPEG", quality=75)
                photo_b64 = base64.b64encode(buf.getvalue()).decode()
            except: photo_b64 = base64.b64encode(ph["photo_data"]).decode()
            ok, msg = supa_post("job_photos", {"job_id": ph["job_id"] or "", "photo_date": ph["photo_date"] or "", "caption": ph["caption"] or "", "photo_data": photo_b64, "uploaded_by": ph["uploaded_by"] or "", "company_id": company_id})
            if ok: local_execute("UPDATE job_photos SET synced=1 WHERE id=?", (ph["id"],))
            else: errors.append(f"photo: {msg}")
...
def get_clock_status(employee, company_id):
    today = date.today().isoformat()
    events = local_fetch("SELECT event_type, event_time, job_id FROM clock_events WHERE employee=? AND event_date=? AND company_id=? ORDER BY id DESC LIMIT 1", (employee, today, company_id))
...
def get_today_hours(employee, company_id):
    ...
    if cin:
        diff = (now_sydney() - cin).total_seconds() / 3600
        if diff > 0: total += diff
...
def lookup_company_code(code):
    ...
    if local: return local[0]["id"], local[0]["name"]
    if USE_SUPABASE:
...
if st.session_state.mobile_user is None:
    st.markdown("<div style='text-align:center;padding:32px 0 20px'><div style='font-size:28px;font-weight:900;letter-spacing:.08em;color:#2dd4bf'>LIMITLESS</div><div style='font-size:11px;color:#475569;letter-spacing:.2em;text-transform:uppercase;margin-top:2px'>JOB MANAGEMENT</div><div style='font-size:11px;color:#2dd4bf;letter-spacing:.2em;text-transform:uppercase'>SITE APP</div></div>", unsafe_allow_html=True)
    ...
    if stage == "company":
        ...
        code_input = st.text_input("Company Code", placeholder="e.g. UTR-0016", max_chars=12, label_visibility="collapsed").upper()
        if st.button("Continue →", type="primary", use_container_width=True):
            if code_input.strip():
                ...
                if cid:
                    ...
                    emps = local_fetch("SELECT name FROM employees WHERE active=1 AND company_id=? ORDER BY name", (cid,))
        ...
        st.markdown("<div style='text-align:center;color:#334155;font-size:12px;margin-top:20px'>Your supervisor will provide your company code</div>", unsafe_allow_html=True)
            ...
            for col, digit in zip(cols, row):
                with col:
                    if st.button(digit, use_container_width=True, key=f"pin_{digit}"):
                        if digit == "←":
                            st.session_state.pin_input = pin[:-1]; st.rerun()
                        elif digit == "✓":
                            emp = local_fetch("SELECT pin FROM employees WHERE name=? AND company_id=? AND active=1", (emp_name, cid))
                            ...
                            if pin == stored_pin:
                                ...
                                st.session_state.pin_input = ""; st.error("❌ Wrong PIN"); st.rerun()
                        else:
                            if len(pin) < 6: st.session_state.pin_input = pin + digit; st.rerun()
        if st.button("← Back to name select"):
            st.session_state.login_stage = "employee"; st.session_state.pin_input = ""; st.rerun()
    st.stop()
...
_sc = "#2dd4bf" if is_clocked_in else "#475569"
_st = "🟢 On Site" if is_clocked_in else "⚫ Off Site"
st.markdown("<div style='display:flex;justify-content:space-between;align-items:center;background:#111c27;border-radius:14px;padding:10px 14px;margin-bottom:14px;border:1px solid #1e2d3d'><div style=\"font-size:20px;font-weight:700;letter-spacing:.06em;color:#2dd4bf\">LIMITLESS</div><div style='text-align:right'><div style='font-size:14px;font-weight:700;color:#e2e8f0'>" + user + "</div><div style='font-size:11px;color:#334155'>" + company_name + "</div><div style='font-size:12px;color:" + _sc + "'>" + _st + " · " + str(today_hours) + "h today</div></div></div>", unsafe_allow_html=True)
...
if page == "home":
    ...
    st.markdown(f"<div style='color:#475569;font-size:13px;margin-bottom:16px'>{date.today().strftime('%A, %d %B')}</div>", unsafe_allow_html=True)

    if is_clocked_in:
        st.markdown("<div style='background:linear-gradient(135deg,#0d2a1f,#1a3a2a);border:2px solid #2dd4bf;border-radius:16px;padding:20px;text-align:center;margin-bottom:16px'><div style='font-size:13px;font-weight:700;color:#2dd4bf;letter-spacing:.1em;text-transform:uppercase'>On Site</div><div style='font-size:48px;font-weight:900;color:#2dd4bf;line-height:1.1'>" + str(today_hours) + "h</div><div style='color:#94a3b8;font-size:13px'>Clocked in at " + (last_time or "")[:5] + " · " + (last_job or "") + "</div></div>", unsafe_allow_html=True)
        if st.button("⏹ Clock Out Now", type="primary"): st.session_state.mobile_page = "clock"; st.rerun()
    else:
    ...
    if not assigned:
        ...
        for job in assigned:
            st_t = str(job["start_time"] or ""); en_t = str(job["end_time"] or "")
            time_str = f"{st_t[:5]} – {en_t[:5]}" if st_t and en_t else ""
            t_line = f"<div style='color:#f59e0b;font-size:13px;margin-top:4px'>🕐 {time_str}</div>" if time_str else ""
            n_line = f"<div style='color:#94a3b8;font-size:13px'>{job['note']}</div>" if job['note'] else ""
            st.markdown(f"<div class='site-card'><div style='font-size:18px;font-weight:800;color:#2dd4bf'>{job['job_id']}</div><div style='color:#e2e8f0;font-size:15px;font-weight:600'>{job['client'] or ''}</div><div style='color:#64748b;font-size:13px;margin-top:4px'>📍 {job['address'] or ''}</div>{t_line}{n_line}</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    _border  = "#2dd4bf" if is_clocked_in else "#2a3d4f"
    _badge   = "status-badge-in" if is_clocked_in else "status-badge-out"
    _slabel  = "🟢 ON SITE" if is_clocked_in else "⚫ OFF SITE"
    _cinline = f"<div style='color:#94a3b8;font-size:13px;margin-top:10px'>Clocked in at {(last_time or '')[:5]} on {last_job or ''}</div>" if is_clocked_in else ""
    _hline   = f"<div style='font-size:28px;font-weight:800;color:#2dd4bf;margin-top:8px'>{today_hours}h today</div>"
    st.markdown(
        "<div style='background:#111c27;border:2px solid " + _border + ";border-radius:16px;padding:24px;text-align:center;margin-bottom:20px'>"
        "<div style='font-size:52px;font-weight:900;color:#e2e8f0;letter-spacing:-.02em'>" + now_str + "</div>"
        "<div style='margin-top:8px'><span class='" + _badge + "'>" + _slabel + "</span></div>"
        + _cinline + _hline + "</div>", unsafe_allow_html=True)

    all_jobs = local_fetch("SELECT job_id, client FROM jobs WHERE stage='Live Job' AND company_id=? ORDER BY job_id", (company_id,))
    if not all_jobs: all_jobs = local_fetch("SELECT job_id, client FROM jobs WHERE company_id=? ORDER BY job_id", (company_id,))
    ...
    assigned_today = local_fetch("SELECT job_id FROM day_assignments WHERE employee=? AND date=? AND company_id=? LIMIT 1", (user, today_str, company_id))
    default_idx = 0
    if assigned_today and assigned_today[0]["job_id"] in job_ids:
        default_idx = job_ids.index(assigned_today[0]["job_id"])
    selected_idx = st.selectbox("Job", range(len(job_options)), format_func=lambda x: job_options[x], index=default_idx)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if photo_file and st.button("Upload Photo", type="primary"):
        local_execute("INSERT INTO job_photos (job_id,photo_date,caption,photo_data,uploaded_by,company_id,synced) VALUES (?,?,?,?,?,?,0)",
            (photo_job, today_str, photo_caption, photo_file.read(), user, company_id))
        errs = sync_to_supabase(user, company_id)
        st.success("✅ Photo saved!" + ("" if not errs else " (will sync later)"))
        st.rerun()
    recent = local_fetch("SELECT caption, photo_date, job_id FROM job_photos WHERE uploaded_by=? AND company_id=? ORDER BY id DESC LIMIT 10", (user, company_id))
    if recent:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("Submit Variation", type="primary"):
        if var_desc.strip():
            local_execute("INSERT INTO mobile_variations (employee,job_id,description,submitted_at,status,company_id,synced) VALUES (?,?,?,?,?,?,0)",
                (user, var_job, var_desc.strip(), now_sydney().isoformat(), "Pending", company_id))
            sync_to_supabase(user, company_id)
            st.success("✅ Variation submitted — office will review and approve.")
            st.balloons()
        else: st.error("Please describe what you found.")
    my_vars = local_fetch("SELECT job_id, description, status FROM mobile_variations WHERE employee=? AND company_id=? ORDER BY id DESC LIMIT 5", (user, company_id))
    if my_vars:
        ...
        for v in my_vars:
            ...
            st.markdown(f"<div class='site-card'><div style='display:flex;justify-content:space-between;align-items:center'><span style='color:#e2e8f0;font-weight:700'>{v['job_id']}</span><span style='color:{sc};font-size:12px;font-weight:700'>{v['status']}</span></div><div style='color:#94a3b8;font-size:13px;margin-top:4px'>{str(v['description'])[:80]}</div></div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if st.button("Update PIN"):
        if new_pin and new_pin == confirm_pin and new_pin.isdigit():
            local_execute("UPDATE employees SET pin=? WHERE name=? AND company_id=?", (new_pin, user, company_id))
            supa_patch("employees", "name", user, {"pin": new_pin})
            st.success("✅ PIN updated!")
        else: st.error("PINs must match and be digits only.")
    st.divider()
    if st.button("Sign Out"):
        for k in ["mobile_user","mobile_company_id","mobile_company_name","mobile_page","login_stage","pin_input","selected_employee"]:
            st.session_state[k] = None if k in ["mobile_user","mobile_company_id","selected_employee"] else ("login" if k == "mobile_page" else ("company" if k == "login_stage" else ""))
        st.rerun()
